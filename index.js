...
const iconFontFolders = getDirectories(__dirname).filter(
  (dir) => !dir.match(/\.git|node_modules|_vault/)
)
...
@font-face {
    ...
    src: url('data:@file/octet-stream;base64,${woff2.toString(
      "base64"
    )}') format('woff2');
...
${glyphs
  .map(
    (g) =>
      `<tr><td>${g.unicode[0]}</td><td>${g.filename}</td><td>U+${g.code}</td><td>${g.unicode[0]}</td></tr>`
  )
  .join("")}</tbody></table>`
      ...
      if (fs.existsSync(cspPath)) {
        ...
        if (fs.existsSync(resolvedCspTargetPath)) {
          ...
          console.log(
            `csp: Copied '${folderName}.css' to ${cspTargetSnippetFilepath}`
          )
        } else {
          console.log(
            `csp: Target path '${resolvedCspTargetPath}' does not exist, ignoring.`
          )
        }
      }
